...
VA_SetVolume(vol, subunit_desc="1", channel="", device_desc="playback")
{
    ...
    if channel =
    {
        ...
        Loop, %channel_count%
        {
            this_vol := channel%A_Index%vol / vol_max * vol
...
VA_GetDevice(device_desc="playback")
{
    ...
    if DllCall(NumGet(NumGet(deviceEnumerator+0)+20), "uint",deviceEnumerator, "uint",COM_Unicode4Ansi(wstr, device_desc), "uint*",device) = 0
...
VA_GetDeviceName(device)
{
    ...
    COM_Release(store)
    return COM_Ansi4Unicode(NumGet(prop,8)), COM_CoTaskMemFree(NumGet(prop,8))
}
...
VA_IMMDevice_GetId(this, ByRef Id) {
    ...
    Id := (COM_Ansi4Unicode(Id),COM_CoTaskMemFree(Id))
    return hr
}
...
VA_IDeviceTopology_GetDeviceId(this, ByRef DeviceId) {
    ...
    DeviceId := (COM_Ansi4Unicode(DeviceId),COM_CoTaskMemFree(DeviceId))
    return hr
}
...
VA_IConnector_GetConnectorIdConnectedTo(this, ByRef ConnectorId) {
    ...
    ConnectorId := (COM_Ansi4Unicode(ConnectorId),COM_CoTaskMemFree(ConnectorId))
    return hr
}
...
VA_IPart_GetName(this, ByRef Name) {
    ...
    Name := (COM_Ansi4Unicode(Name),COM_CoTaskMemFree(Name))
    return hr
}
...
VA_IPart_GetGlobalId(this, ByRef GlobalId) {
    ...
    GlobalId := (COM_Ansi4Unicode(GlobalId),COM_CoTaskMemFree(GlobalId))
    return hr
}
